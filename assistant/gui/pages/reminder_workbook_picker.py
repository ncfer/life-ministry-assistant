...
from ..widgets import NavButton, REMINDER_STEPS, SourceCard, StepHeader
from ..workers import (
    DownloadReminderWorkbookDriveThread, DownloadReminderWorkbookPadletThread,
    ParseReminderWorkbookThread,
)


class ReminderWorkbookPickerPage(QWidget):
    def __init__(self, main_window):
        super().__init__()
        self.main_window = main_window
        self._thread: (
            ParseReminderWorkbookThread | DownloadReminderWorkbookPadletThread
            | DownloadReminderWorkbookDriveThread | None
        ) = None
    ...
    def _choose_file(self) -> None:
        ...
        self.main_window.reminder_state.workbook_path = path
        self.main_window.reminder_state.reset_from_workbook()

        self._set_buttons_enabled(False)
        self.progress_bar.show()
        self._thread = ParseReminderWorkbookThread(path)
        self._thread.done.connect(lambda weeks: self._on_done(weeks, {f: path for f in weeks}))
    ...
    def _search_padlet(self) -> None:
        ...
        self._thread = DownloadReminderWorkbookPadletThread(padlet, folder)
        self._thread.done.connect(self._on_padlet_downloaded)
        self._thread.error.connect(self._on_padlet_error)
        self._thread.start()

    def _on_padlet_downloaded(self, weeks: dict, paths_by_date: dict) -> None:
        first_path = next(iter(paths_by_date.values()), None)
        self.file_label.setText(
            t("workbook_picker.descargado_padlet", nombre=first_path.name) if first_path
            else t("workbook_picker.descargado_padlet_generico")
        )
        self.main_window.reminder_state.workbook_path = first_path
        self.main_window.reminder_state.reset_from_workbook()
        self._on_done(weeks, paths_by_date)
    ...
    def _search_drive(self) -> None:
        ...
        self.progress_bar.show()
        folder = Path(self.main_window.config.paths.output_folder)
        self._thread = DownloadReminderWorkbookDriveThread(drive_link, folder)
        self._thread.done.connect(self._on_drive_downloaded)
        self._thread.error.connect(self._on_drive_error)
        self._thread.start()

    def _on_drive_downloaded(self, weeks: dict, paths_by_date: dict) -> None:
        first_path = next(iter(paths_by_date.values()), None)
        self.file_label.setText(
            t("workbook_picker.descargado_drive", nombre=first_path.name) if first_path
            else t("workbook_picker.ningun_archivo")
        )
        self.main_window.reminder_state.workbook_path = first_path
        self.main_window.reminder_state.reset_from_workbook()
        self._on_done(weeks, paths_by_date)
    ...
    def _on_done(self, weeks: dict, paths_by_date: dict) -> None:
        self._set_buttons_enabled(True)
        self.progress_bar.hide()
        self.main_window.reminder_state.weeks = weeks
        self.main_window.reminder_state.workbook_paths_by_date = paths_by_date
        self.main_window.go_to(8)  # -> pick week
